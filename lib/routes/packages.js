...
var uniqueURL = require('../uniqueURL');
...
function packageRegistered(response, message) {
  message = message || 'Package already registered';

  serverStatus.errors.createPackageQuery++;
  return response.send(403, message);
}
...
exports.create = function (request, response) {
    ...
    if (validation.error) {
          ...
          return response.send(400, 'Invalid Package Name. ' + validation.error);
    }

    validateURL(url, function(isValidURL) {
        if (isValidURL) {
            uniqueURL(url, function(err, isDupe, dupe) {
                if (err) {
                  response.send('500', err);
                }
                if (isDupe) {
                    var message = url + ' is already registered as ' + dupe.name;
                    return packageRegistered(response, message);
                }
                database.insertPackage(name, url, function (error) {
                    if (error) {
                        packageRegistered(response);
                    }
                    response.send(201);
                });
            });
