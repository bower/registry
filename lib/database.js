...
exports.searchPackages = function (term, limit, callback) {
    if (term) {
        query('SELECT name, url FROM packages WHERE name ILIKE $1 OR url ILIKE $1 ORDER BY length(name) ASC LIMIT $2', ['%' + term + '%', limit], callback);
    } else {
        query('SELECT name, url FROM packages ORDER BY name DESC LIMIT $1', [limit], callback);
    }
};
