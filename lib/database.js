...
var connectionParams = (exports.connectionParams = parseDatabaseUrl(
  connectionString
));
...
exports.searchPackages = function(term, limit, callback) {
  if (term) {
    query(
      'SELECT name, url FROM packages WHERE name ILIKE $1 OR url ILIKE $1 ORDER BY similarity(name, $3) DESC LIMIT $2',
      ['%' + term + '%', limit, term],
      callback
    );
  } else {
    query(
      'SELECT name, url FROM packages ORDER BY hits DESC LIMIT $1',
      [limit],
      callback
    );
  }
...
exports.truncate = function(callback) {
  ...
  if (process.env.NODE_ENV == 'test') {
    query(
      'TRUNCATE packages RESTART IDENTITY',
      removeCacheAndReturn('packages', callback)
    );
  } else {
    removeCacheAndReturn('packages', callback)();
...
exports.dropDatabase = function() {
  ...
  return adminQuery(
    'DROP DATABASE ' + connectionParams.database
  ).then(null, function(error) {
    console.error(error.message);
  });
};
